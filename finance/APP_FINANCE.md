...
- `date_fin` (paramètre, `aujourd'hui` par défaut) permet uniquement de consulter *"quel était le solde à telle date"* — borné en bas par `_date_debut_effective(date_fin)`, pas directement par `DATE_DEBUT_FINANCE` (voir ci-dessous).
- **Nouvelle période, sans report d'ouverture (décision mdmaiga, 24/09/2026)** : `DATE_DEBUT_PERIODE_ACTUELLE` (`date(2026, 10, 1)`) est une seconde constante de module. `_date_debut_effective(date_fin)` retourne `DATE_DEBUT_PERIODE_ACTUELLE` si `date_fin` s'y situe, sinon `DATE_DEBUT_FINANCE` (comportement historique inchangé pour consulter une période antérieure déjà close). Contrairement à la coupure du 01/08/2026, celle-ci n'a **aucun** `ajustement_solde` de report : la formule met `ajustement=0` dès que la borne basse effective est `DATE_DEBUT_PERIODE_ACTUELLE`, quelle que soit la valeur du champ (qui reste l'ouverture historique du 01/08, non réutilisée ici). `solde_caisse_globale` applique la même borne. **Volontairement pas répliqué ailleurs** : `agents.services.rot_dashboard_service.RotDashboardService.get_cash_superviseur_post_cloture` (écran ROT `recouvrer_superviseur`) a son propre mécanisme basé sur `ClotureMensuelle`, mais **constat du 24/09/2026 (mdmaiga)** — ce mécanisme est obsolète en pratique : `cloturer_mois` n'a plus tourné depuis juin 2026 (dernière `ClotureMensuelle` en base : 30/06/2026), jamais corrigé pour s'exécuter en routine. Un premier correctif y avait été appliqué le 24/09/2026 (même plancher `DATE_DEBUT_PERIODE_ACTUELLE`) puis annulé le jour même à la demande de mdmaiga — pas la peine de maintenir à jour un calcul déjà considéré obsolète. Voir `agents/APP_AGENT.md` § 7. Les 5 propriétés `Agent.solde_*` legacy (`core/models.py`, cf. tableau « Frontières ») n'ont pas non plus été touchées : même famille de mécanisme `ClotureMensuelle`, même constat.
