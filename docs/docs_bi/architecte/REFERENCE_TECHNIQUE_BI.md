...
2. Agents ciblés : `paie.services.agent_eligibilite.agents_eligibles_periode(date_debut, date_fin)` — **plus un simple `Agent.objects.filter(est_actif=True, ...)` depuis le 05/08/2026** (correction documentée dans `paie/APP_PAIE.md` §1.ter) : un agent `est_actif=True` est toujours éligible quelles que soient ses dates de contrat ; un agent désactivé reste éligible seulement si sa fenêtre d'emploi connue (`date_debut_fonction` → `date_fin_contrat`) chevauche `[date_debut, date_fin]` (rattrapage historique). Règle reproduite dans `dbt_bi/models/marts/fct_salaires.sql` (réalignée le 06/08/2026, elle reposait auparavant sur l'ancienne philosophie liée aux dates de contrat).
